...
def run(generations_file, watermarks_file):
    ...
    with open(watermarks_file) as infile:
        watermarks = {str(replace(WatermarkSpec.from_str(l.strip()), tokenizer=model).to_dict(True, True)) for l in
                      infile.read().split("\n") if len(l)}

    filt_gens = [g for g in generations if g.watermark if
                 not None and str(g.watermark.to_dict(True, True)) in watermarks]
