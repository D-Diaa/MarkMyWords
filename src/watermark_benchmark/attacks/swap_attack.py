...
class SwapAttack(Attack):
    ...
    def __init__(self, word_repeat_prob=0.5, word_delete_prob=0.5, word_swap_probability=0.5,
                 sentence_swap_probability=0.5):
    ...
    def warp(self, text, input_encodings=None):
        ...
        if self.word_swap_probability > 0:
            for i in range(len(sentences)):
                words = [(match.group(), match.start(), match.end()) for match in
                         re.finditer(self.word_regex, sentences[i])]
                if len(words) < 2:
                    continue
